...
# print(f"{doc["Data"].keys() = }")

# print(f"{doc['Metadata']["Data Parameters"]}")
# print(f"{doc['Data']["Array Layout"].keys() = }")
# data = doc['Data']["Array Layout"]["Layout Description"]
# # print(array(data))
...
# TRO_lon = 180 + int(18.9553)
# TRO_lat = 90 + int(69.6492)
# lats = np.linspace(0, 180, 180) 
# lons = np.linspace(0, 360, 360) 

# All data is binned by 1 deg lon and lat.
# Assumption: 0 index means at 0 lat/lon. index [50, 65] means 50 deg lon and 65 deg lat


# time_expriment_start_min = 18*60
# time_expriment_end_min = 24*60
...
def plot_reference_boxes(lat_widths, lon_widths, fig, ax):
    mesh = ax.pcolormesh(
        lons,
        lats,
        data["tec"][
...
# lon_widths = range(1, 24, 4)
# lat_widths = [6]*len(lon_widths)
# plot_time_series(lat_widths=lat_widths, lon_widths=lon_widths, fig=fig, ax=axs[0])
# plot_reference_boxes(lat_widths=lat_widths, lon_widths=lon_widths, fig=fig, ax=axs[1])
# title="Longitudinal change"
# fig.suptitle("TEC at Tromsø over time" + "\n" + title)
# fig.tight_layout()  
# fig.savefig("figures\\TEC\\TEC over time " + title + ".png")

# fig, axs=subplots(1, 2, figsize=(10, 8))
# lat_widths = range(1, 24, 4)
# lon_widths = [6]*len(lon_widths)
# plot_time_series(lat_widths=lat_widths, lon_widths=lon_widths,  fig=fig, ax=axs[0])
# plot_reference_boxes(lat_widths=lat_widths, lon_widths=lon_widths,  fig=fig, ax=axs[1])
# title="Latitudinal change"
# fig.suptitle("TEC at Tromsø over time" + "\n" + title)
# fig.tight_layout()
# fig.savefig("figures\\TEC\\TEC over time " + title + ".png")
...
def load_naive(time_format):
    # gns_path = r"data\gps260202g.001.hdf5"
    gns_path = r"C:\Users\NoahH\OneDrive - UiT Office 365\Personal OneDrive copy\Tromsø\UiT Arctic University of Norway\Techniques for Investigating the Near-Earth Space Environment\HEIMDALL\data_gathering\src\data\TEC\gps260202g.001.hdf5"
    doc = h5py.File(gns_path, "r")
    data = doc['Data']["Array Layout"]["2D Parameters"]
    data_param = doc['Metadata']["Experiment Notes"]
    print(array(data_param))
...
def make_animation():
    ...
    ax.set_title(f"{DT.hour}:{DT.minute}")
    ...
    def update(i):
        ...
        DT = datetime.datetime.fromtimestamp(time_unix)
        ax.set_title(f"{DT.hour}:{DT.minute}")
    anim = animation.FuncAnimation(fig, update, TEC.shape[2], interval=50)
    # anim.save("figures\\TEC\\day animation.gif")
    show()
make_animation()